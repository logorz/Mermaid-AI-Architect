...
interface ChatPanelProps {
  ...
  isLoading: boolean;
  onOptimizeText?: (text: string) => Promise<string>;
}

export const ChatPanel: React.FC<ChatPanelProps> = ({ history, onSendMessage, isLoading, onOptimizeText }) => {
    ...
    <div className="flex flex-col h-full bg-white dark:bg-slate-900 border-r border-slate-200 dark:border-slate-700 transition-colors duration-300">
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4">
        ...
        {history.map((msg) => (
          ...
          >
            {/* Show generate flowchart button for message type responses */}
            {msg.role === 'model' && msg.type === 'message' && (
              <div className="mb-2 w-full">
                <button 
                  onClick={() => onSendMessage(`基于以下对话内容生成流程图：${msg.content}`, undefined, 'auto')}
        ...
        <form onSubmit={handleSubmit} className="relative flex flex-col gap-2">
          ...
          <div className="absolute right-2 bottom-2 flex items-center space-x-1">
            ...
            <button
              ...
              title="上传图片或 PDF"
            >
              {selectedFile ? <span>📋</span> : <span>📷</span>}
            </button>
            {onOptimizeText && (
              <button
                type="button"
                ...
                title="AI 优化需求描述"
              >
                <span className={`w-5 h-5 flex items-center justify-center ${isOptimizing ? 'animate-spin' : ''}`}>✨</span>
              </button>
            )}
            <button
              type="submit"
